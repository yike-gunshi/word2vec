...
class CBOW():
    def __init__(self, tran_flag=False):
        self.data_index = 0
    ...
    def generate_batch(self, batch_size, skip_window, data):
        ...
        # skip_window:单词最远可以联系的距离（本次实验设为2，即目标单词只能和相邻的两个单词生成样本），2*skip_window>=num_skips
        ...
        for i in range(batch_size):
            ...
            col_idx = 0
            for j in range(span):
                if j == span // 2:  # 如果j是目标词，不执行以下操作
                    continue
                batch[i, col_idx] = buffer[j]  # (batch_size, span - 1) 第i个样本的第j列=窗口内的第j个元素
                col_idx += 1
            labels[i, 0] = buffer[target]   # (batch_size, 1)  第i行是中心词

            buffer.append(data[self.data_index])
            self.data_index = (self.data_index + 1) % len(data)

        assert batch.shape[0] == batch_size and batch.shape[1] == span - 1  # 确认行和列的大小
        # assert断言：申明其布尔值必须为真的判定，如果发生异常，就表示为假
        return batch, labels
        #  batch的行：样本  列：上下文单词数（C=2A）
    ...
    def train_wordvec(self, vocabulary_size, batch_size, embedding_size, window_size, num_sampled, num_steps, data):
        ...
        with tf.Session(graph=graph) as session:
            ...
            for step in range(num_steps):
                ...
                final_embeddings, l, n, a_e, s_w, s_b = session.run([optimizer, loss, normalized_embeddings, avg_embed, softmax_weights, softmax_biases], feed_dict=feed_dict)
                average_loss += l
                if step % 2000 == 0:
                    if step > 0:
    ...
    def train(self):
        ...
        final_embeddings,avg_embed, softmax_weights, softmax_biases = self.train_wordvec(vocabulary_size, self.batch_size, self.embedding_size, self.window_size, self.num_sampled, self.num_steps, data)
        # final_embeddings = self.train_wordvec(vocabulary_size,
        #                                                                                   self.batch_size,
        #                                                                                   self.embedding_size,
        #                                                                                   self.window_size,
        #                                                                                   self.num_sampled,
        ...
        with tf.Session() as sess:
            logits0 = tf.matmul(avg_embed, tf.transpose(softmax_weights))
            logits = tf.nn.bias_add(logits0, softmax_biases)
            logits = logits.eval(session=sess)[1, :]
            # (100, 881)
            softmax = tf.nn.softmax(logits)
            # (100, 881)
            softmax = softmax.eval(session=sess)
            # print('logits:')
            # print(logits.shape)
            # print(logits)
            # print('softmax:')
        ...
        num = [i for i in softmax]
        ls = 0
        for i in num:
            ls += i
        print(ls)
        # logits0 = tf.matmul(self.avg_embed, tf.transpose(self.softmax_weigths))
        # logits = tf.nn.bias_add(logits0, self.softmax_biases)
        # softmax = tf.nn.softmax(logits)
        # sess = tf.Session()
        # l =  tf.convert_to_tensor(logits)
        # s =  tf.convert_to_tensor(softmax)
        #
        # print('logits:')
        # print(l)
        # print('softmax:')
    ...
    def jiucuoceshi(self, sentence='白色地危化品怎么洗硝'):
        """需要用到的参数：
        """
        data, count, dictionary, reverse_dictionary = self.build_dataset(self.words, self.min_count)
        vocabulary_size = len(count)
        final_embeddings, avg_embed, softmax_weights, softmax_biases = self.train_wordvec(vocabulary_size, self.batch_size, self.embedding_size, self.window_size, self.num_sampled, self.num_steps, data)
        # 找到每个字的one-hot向量
        # 用上下文的one-hot，乘以W（final_embeddings），得到嵌入向量矩阵
        # 取平均
        # 乘以W'（softmax_weights），加上b（softmax_biases），做softmax
        # 根据softmax向量和目标字的one-hot向量，判断是否对应于top3的字
